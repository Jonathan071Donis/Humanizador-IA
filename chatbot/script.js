...
const welcomeMessage =
  "Hola! Soy tu asistente virtual. Puedo responder tus preguntas y mostrarte imagenes. Si quieres imagenes, solo dime 'muestrame imagenes de...' y te preguntare cuantas deseas.";
...
async function sendMessage() {
  ...
  try {
    ...
    if (sanitizedMessage.toLowerCase().includes("muestrame imagenes de")) {
      ...
      imageQuery = sanitizedMessage
        .toLowerCase()
        .replace("muestrame imagenes de", "")
        .trim();
      ...
      if (!imageQuery) {
        appendMessage(
          "bot",
          'Por favor, especifica que imagenes deseas ver. Por ejemplo: "Muestrame imagenes de un leon".'
        );
        return;
      }
...
async function fetchAndDisplayImages(query, count) {
  try {
    const unsplashResponse = await fetch(
      `${UNSPLASH_API_URL}?query=${encodeURIComponent(query)}&per_page=${count}&client_id=${UNSPLASH_API_KEY}`
    );
...
window.addEventListener("resize", () => {
  const chatContainer = document.querySelector(".chat-container"); // Asegurate de seleccionar el contenedor del chat
  if (!chatContainer) return; // Si no existe el contenedor, salimos de la funcion
...
window.addEventListener("load", () => {
  ...
  if (!bubbleMessage) return; // Si no existe el mensaje, salimos de la funcion

  bubbleMessage.style.display = "block"; // Muestra el mensaje
  setTimeout(() => {
    bubbleMessage.style.display = "none"; // Oculta el mensaje despues de 5 segundos
  }, 5000);
});